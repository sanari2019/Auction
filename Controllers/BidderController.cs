...
public class BidderController : ControllerBase
{
    ...
    public async Task<ActionResult<Bidder>> GetBidder(string username)
    {
        return new OkObjectResult(_repBidder.GetBidder(username));
    }
    // PUT: api/users/5
                ...
                using (PrincipalContext pc = new PrincipalContext(ContextType.Domain, "evercare.ng"))
                {
                    ...
                    if (isValid == true)
                    {
                        ...
                        if (testAvail == true)
                        {
                            ...
                            {
                                if (bidder.username == loginUser.username)
                                {
                                    us=bidder;
